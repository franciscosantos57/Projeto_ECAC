...
def load_all_participants_data(dataset_path="dataset"):
    ...
    Adiciona uma coluna extra com o participant_id.
    
    Returns:
        tuple: (dados_combinados, info_participantes)
            - dados_combinados: Array com todos os dados [n_samples, 13] 
              (12 colunas originais + participant_id)
    ...
    for participant_id in range(15):
        try:
            ...
            if len(participant_data) > 0:
                # Adiciona coluna com participant_id
                participant_column = np.full((len(participant_data), 1), participant_id)
                participant_data_with_id = np.hstack([participant_data, participant_column])
                
                all_data.append(participant_data_with_id)
